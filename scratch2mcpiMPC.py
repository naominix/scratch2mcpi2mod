    ...
    for msg in _listen(s):
        if (msg):
            ...
            if msg[0] == 'broadcast':
                ...
                if not localmc:
                    localmc = minecraft.Minecraft.create( )
                if msg[1] == 'hello_minecraft':
                    mc.postToChat("hello minecraft")
                    mc.postToChat("hello local minecraft")
                elif msg[1] == 'setPos':
                ...
                # Minecraft Stuff(End)
                    ...
                    playerPos = localmc.player.getPos()
                    pos = playerPos
                    steve = turtle.MinecraftTurtle(mc, pos)
                    s.sensorupdate({
                        'playerX': playerPos.x,
...
def main():
    ...
    argc = len(argvs)
    if (argc != 2):
	      print 'Usgae: python %s MCPI-Server-IP' % argvs[0]
	      quit()

    server = argvs[1]
    print "Server IP Addr= %s " % argvs[1] 
    print "================="
    ...
    while True:
        ...
        if (s):
            mc.postToChat("Remote Scratch2MCPI connected to Minecraft Pi.")
            localmc.postToChat("Local Scratch2MCPI connected to Minecraft Pi.")
